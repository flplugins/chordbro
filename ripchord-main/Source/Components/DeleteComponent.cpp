...
{
    Styles::drawPanel (inGraphics, getLocalBounds().toFloat(), 16.f, COLOR_PANEL_RAISED);

    auto accent = getLocalBounds().toFloat();
    accent.setHeight (4.f);
    inGraphics.setColour (COLOR_RED.withAlpha (0.22f));
    inGraphics.fillRoundedRectangle (accent, 16.f);
}
